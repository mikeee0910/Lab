...
#define ADC_EVENT_HALF      0x0001U
#define ADC_EVENT_FULL      0x0002U
...
void HAL_ADC_ConvHalfCpltCallback(ADC_HandleTypeDef *hadc)
{
  ...
  {
    osThreadFlagsSet(myTask02Handle, ADC_EVENT_HALF);
  }
}
...
void HAL_ADC_ConvCpltCallback(ADC_HandleTypeDef *hadc)
{
  if (hadc->Instance == ADC1)
  {
    osThreadFlagsSet(myTask02Handle, ADC_EVENT_FULL);
  }
}
...
void StartTask02(void *argument)
{
  ...
  for(;;)
  {
	flags = osThreadFlagsWait(ADC_EVENT_HALF | ADC_EVENT_FULL,
							  osFlagsWaitAny,
							  osWaitForever);

	if ((flags & ADC_EVENT_HALF) != 0U)
	{
	  printf("=== Half buffer filled ===\r\n");

	  for (int i = 0; i < SAMPLE_BUFFER_SIZE / 2; i++)
	  {
		printf("[Half] buffer[%d] = %u\r\n", i, sample_buffer[i]);
	  }
	}

	if ((flags & ADC_EVENT_FULL) != 0U)
	{
	  printf("=== Full buffer filled ===\r\n");

	  for (int i = SAMPLE_BUFFER_SIZE / 2; i < SAMPLE_BUFFER_SIZE; i++)
	  {
		printf("[Full] buffer[%d] = %u\r\n", i, sample_buffer[i]);
	  }
	}
  }
