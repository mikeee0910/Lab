...
int main(void)
{
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	__HAL_TIM_SET_COMPARE(&htim2, TIM_CHANNEL_1, 249999); // 25%
	HAL_Delay(2000);

	__HAL_TIM_SET_COMPARE(&htim2, TIM_CHANNEL_1, 499999); // 50%
	HAL_Delay(2000);

	__HAL_TIM_SET_COMPARE(&htim2, TIM_CHANNEL_1, 749999); // 75%
	HAL_Delay(2000);

//	__HAL_TIM_SET_COMPARE(&htim2, TIM_CHANNEL_1, 999999); // 100%
//	HAL_Delay(2000);
  }
  /* USER CODE END 3 */
}
...
void HAL_TIM_IC_CaptureCallback(TIM_HandleTypeDef *htim)
{
    if (htim->Instance == TIM3 && htim->Channel == HAL_TIM_ACTIVE_CHANNEL_1)
    {
        ...
        else if (cap_state == 2)
        {
            ...
            if (fall1 >= rise1)
                high_count = fall1 - rise1;
            else
                high_count = (65535 - rise1) + fall1 + 1;

            if (rise2 >= rise1)
                period_count = rise2 - rise1;
            else
                period_count = (65535 - rise1) + rise2 + 1;

            // TIM3 prescaler = 799, so 1 count = 10 us = 0.01 ms
            high_time_ms = high_count * 0.01f;
            period_ms = period_count * 0.01f;

            duty_percent = ((float)high_count / (float)period_count) * 100.0f;
            frequency_hz = 100000.0f / (float)period_count;
