...
volatile uint16_t g_sampling_freq = 10;
      ...
      case EVT_BLUE_GATT_ATTRIBUTE_MODIFIED:
            {
              ...
              /* AccFreqCharHandle + 1 = value attribute handle */
              if (am->attr_handle == AccFreqCharHandle + 1 &&
                  am->data_length == 2)
              {
                uint16_t new_freq = am->att_data[0] |
                                    ((uint16_t)am->att_data[1] << 8);
                if (new_freq >= 1 && new_freq <= 100) {
                  g_sampling_freq = new_freq;
                  printf("Sampling freq updated: %d Hz\r\n", g_sampling_freq);
                }
              }
            }
            break;
